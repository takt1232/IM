...
?>
    ...
    <?php
    $sql = "SELECT DISTINCT store_id, store_name FROM store";
    $stmt = $pdo->query($sql);
    $opt = "<select class='store-opt' name='store-owner-opt' id='store-opt' required>
        <option disabled selected>Select Store</option>";
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $opt .= "<option value='" . $row['store_id'] . "'>" . $row['store_name'] . "</option>";
    }
    $opt .= "</select>";
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><a href='#' onclick='openEditOrderPopup(<?php echo json_encode($orderRow); ?>)' class='edit'>
